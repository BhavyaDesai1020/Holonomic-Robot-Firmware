...
PIDController headingPID(HEADING_KP, HEADING_KI, HEADING_KD);
...
void emergencyStop() {
  ...
  pid1.reset(); pid2.reset(); pid3.reset(); headingPID.reset();
...
void handleSerialTuning() {
    ...
    cmd.trim();
    char tag[10]; float value;
    if (sscanf(cmd.c_str(), "%s %f", tag, &value) < 2) return;

    // Outer Loop (Heading) Tuning
    if (cmd.startsWith("HP")) headingPID.Kp = value;
    else if (cmd.startsWith("HI")) { 
        headingPID.Ki = value; 
        headingPID.i_max = (value != 0) ? 100.0f / value : 0; 
    }
    else if (cmd.startsWith("HD")) headingPID.Kd = value;

    Serial.printf("HEADING PID: P=%.3f I=%.3f D=%.3f\n", 
...
void loop() {
  ...
  if (PS4.isConnected()) {
    ...
    if (currentShareState && !lastShareState) {
      ...
      pid1.reset(); pid2.reset(); pid3.reset(); headingPID.reset();sp1_r = 0; sp2_r = 0; sp3_r = 0;
      setAllMotors(0, 0, 0); digitalWrite(light, LOW);
      ...
      if (robot_ready) {
        ...
        if (L2 > 10 || R2 > 10) { w = (L2 - R2) / 2; target_heading = current_heading; } 
          ...
          if (fabsf(error) > HEADING_DEADZONE) {
            int raw_w = (int)headingPID.compute(0, error * ROTATION_DIR);
